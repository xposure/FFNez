...
138,236,205,93,222,114,67,29,24,72,243,141,128,195,78,66,215,61,156,180};
    ...
    public static float noise(int octaves, float xin, float yin)
    {
        float value = 0.0f;
        int i;
        for (i = 0; i < octaves; i++)
        {
            value += noise(
                xin * (float)Math.Pow(2, i),
                yin * (float)Math.Pow(2, i)
            );
        }
        return value;
    }

    public static float noise(int octaves, float frequency, float xin, float yin)
    {
        return noise(octaves, frequency, frequency, xin, yin);
    }

    public static float noise(int octaves, float freqx, float freqy, float xin, float yin)
    {
        return noise(octaves, xin * freqx, yin * freqy);
    }
