...
namespace Atma.Verlet
{
	...
	{
		public Tree( vec2 origin, int depth = 5, float branchLength = 70, float theta = 0.4f, float segmentCoef = 0.95f )
		{
			var root = addParticle( new Particle( origin ) ).pin();
			var trunk = addParticle( new Particle( origin + new vec2( 0, 10 ) ) ).pin();

			var firstBranch = createTreeBranch( root, 0, depth, segmentCoef, new vec2( 0, -1 ), branchLength, theta );
			addConstraint( new AngleConstraint( trunk, root, firstBranch, 3 ) );

			// animates the tree at the beginning
			var noise = 3;
			for( var i = 0; i < particles.length; ++i )
				particles.buffer[i].position += ( new vec2( Mathf.floor( Random.nextFloat() * noise ), Mathf.floor( Random.nextFloat() * noise ) ) );
		}


		Particle createTreeBranch( Particle parent, int i, int nMax, float segmentCoef, vec2 normal, float branchLength, float theta )
		{
			...
			if( i < nMax )
			{
				var aRot = Mathf.rotateAround( normal, vec2.Zero, -theta * Mathf.rad2Deg );
				var bRot = Mathf.rotateAround( normal, vec2.Zero, theta * Mathf.rad2Deg );
