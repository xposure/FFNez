...
namespace Nez.PhysicsShapes
{
	...
	{
		public static bool circleToCircleCast( Circle first, Circle second, vec2 deltaMovement, out RaycastHit hit )
		{
			hit = new RaycastHit();
			...
			var closestDistanceSquared = vec2.DistanceSquared( second.position, d );
			...
			{
				var normalizedDeltaMovement = vec2.Normalize( deltaMovement );
				...
				if( d == endPointOfCast )
				{
					...
					closestDistanceSquared = vec2.DistanceSquared( second.position, d );
				}

				var backDist = Mathf.sqrt( sumOfRadiiSquared - closestDistanceSquared );

				hit.centroid = d - backDist * normalizedDeltaMovement;
				hit.normal = vec2.Normalize( hit.centroid - second.position );
				hit.fraction = ( hit.centroid.x - first.position.x ) / deltaMovement.x;
				vec2.Distance( ref first.position, ref hit.centroid, out hit.distance );
		...
		public static bool circleToCircle( Circle first, Circle second, out CollisionResult result )
		{
			...
			var distanceSquared = vec2.DistanceSquared( first.position, second.position );
			var sumOfRadii = first.radius + second.radius;
			var collided = distanceSquared < sumOfRadii * sumOfRadii;
			if( collided )
			{
				result.normal = vec2.Normalize( first.position - second.position );
				...
				//result.point = new vec2( collisionPointX, collisionPointY );
		...
		public static bool circleToBox( Circle circle, Box box, out CollisionResult result )
		{
			...
			float sqrDistance;
			vec2.DistanceSquared( ref closestPointOnBounds, ref circle.position, out sqrDistance );
		...
		static bool circleToPolygon2( Circle circle, Polygon polygon, out CollisionResult result )
		{
			...
			var poly2Circle = circle.position - polygon.position;
			var poly2CircleNormalized = vec2.Normalize( poly2Circle );
			var max = float.MinValue;

			for( var i = 0; i < polygon.points.Length; i++ )
			{
				var projection = vec2.Dot( polygon.points[i], poly2CircleNormalized );
				if( max < projection )
				{
					closestPointIndex = i;
					max = projection;
			...
			vec2.DistanceSquared( ref circleCenter, ref closest1, out distance1 );
			vec2.DistanceSquared( ref circleCenter, ref closest2, out distance2 );
			...
			if( distance1 < distance2 )
			{
				...
				result.normal = new vec2( edge.y, -edge.x );
		...
		public static vec2 closestPointOnLine( vec2 lineA, vec2 lineB, vec2 closestTo )
		{
			var v = lineB - lineA;
